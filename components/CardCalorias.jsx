...
const CardCalorias = ({ registro }) => {
    ...
    return (
        <div className="w-full p-6 bg-white border border-gray-200 rounded-lg shadow-md my-2">
            ...
            <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 ">{registro.nombre}</h5>
            <p className="font-semibold">{sumatoriaCalorias}</p>
            <p className="font-semibold">{sumatoriaProteinas}</p>
            <p className="font-semibold">{sumatoriaGrasas}</p>
            </button>
            { productosRutina.length === 0 ? 
                <p>{'No hay productos añadidos a este registro'}</p>
                :
